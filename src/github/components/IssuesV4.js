...
class IssuesV4 extends GithubObject {
  ...
  getSchema() {
    ...
    return new Promise((resolve, reject) => {
      const tablePromises = [
        ...
        Promise.resolve($.getJSON('/github/schema/issue_creators_v4.json')),
  ...
  processData(result, table, rawData) {
    ...
    return new Promise((resolve, reject) => {
      ...
      _.forEach(rawData, (obj) => {
        ...
        // Issue Creators.
        if (_.has(obj, 'author')) {
          if(!_.find(result.users, {id: obj.author.id})) {
            result['users'].push(obj.author);
          }

          result['issue_creators'].push({
            'parent_id': obj.id,
  ...
  formatUserData(user) {
    return {
      ...
      site_admin: user.isSiteAdmin,
      location: user.location,
      name: user.name,
      resource_path: user.resourcePath,
  ...
  formatResponse(response) {
    ...
    for (const { node: result } of results) {
      ...
      data.push({
        ...
        author: this.formatUserData(result.author),
        assignees: this.formatUsersData(result.assignees.nodes),
        comments,
        labels,
      });
    }
